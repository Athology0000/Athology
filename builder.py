...
class MakeEnv:
    ...
    def get_src(self) -> None:
        ...
        """
        subprocess.run(
            ['git', 'clone', 'https://github.com/Athology0000/Athology.git'], cwd=self.build_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        shutil.move(os.path.join(self.build_dir,
                    'Athology', 'src'), self.build_dir)
